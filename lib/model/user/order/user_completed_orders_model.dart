class UserCompletedOrdersModel {
  final int status;
  final String message;
  final List<Order> orders;

  UserCompletedOrdersModel({
    required this.status,
    required this.message,
    required this.orders,
  });

  factory UserCompletedOrdersModel.fromJson(Map<String, dynamic> jsonData) {
    return UserCompletedOrdersModel(
      status: jsonData['status'],
      message: jsonData['message'],
      orders: List<Order>.from(jsonData['order'].map((x) => Order.fromJson(x))),
    );
  }
}
